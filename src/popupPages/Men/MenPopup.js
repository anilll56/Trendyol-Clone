import React from 'react'
import './menPopup.css'

function MenPopup() {
  return (
    <div className='bigContent1'>
      <div className='div1'>
        <h4>Giyim</h4>
        <p>Tişört</p>
        <p>Şort</p>
        <p>Gömlek</p>
        <p>Eşofman</p>
        <p>Pantolon</p>
        <p>Ceket</p>
        <p>Kot Pantolon</p>
        <p>Yelek</p>
        <p>Kazak</p>
        <p>Mont</p>
        <p>Takım Elbise</p>
        <p>Sweatshirt</p>
        <p>Forma</p>
      </div>
      <div className='div2'>
        <div>
          <h4>Ayakkabı</h4>
          <p>Spor Ayakkabı</p>
          <p>Günlük Ayakkabı</p>
          <p>Yürüyüş Ayakkabısı</p>
          <p>Krampon</p>
          <p>Sneaker</p>
          <p>Koşu & Antreman</p>
        </div>
        <div>
          <h4>Kişisel Bakım</h4>
          <p>Parfüm</p>
          <p>Cinsel Sağlık</p>
          <p>Traş Sonrası Ürünler</p>
          <p>Traş Bıçağı</p>
        </div>
      </div>
      <div className='div3'>
        <div>
          <h4>Saat & Aksesuar</h4>
          <p>Saat</p>
          <p>Güneş Güzlüğü</p>
          <p>Cüzdan</p>
          <p>Kemer</p>
          <p>Çanta</p>
          <p>Şapka</p>
          <p>Bileklik</p>
        </div>
        <div>
          <h4>İç Giyim</h4>
          <p>Boxer</p>
          <p>Çorap</p>
          <p>Pijama</p>
          <p>Atlet</p>
        </div>
      </div>
      <div className='div5'>
        <h4>Spor & Outdoor</h4>
        <p>Sweatshirt</p>
        <p>T-shirt</p>
        <p>Forma</p>
        <p>Eşofman</p>
        <p>Koşu Ayakkabısı</p>
        <p>Spor Çantası</p>
        <p>Spor Ekipmanları</p>
        <p>Outdoor Ayakkabı</p>
        <p>Kar Botu</p>
        <p>Outdoor Ekipmanları</p>
        <p>Sporcu Besinleri</p>
        <p>Sporcu Aksesuarları</p>
      </div>
      <div className='div4'>
        <div>
            <h4>Elektronik</h4>
            <p>Traş Makinesi</p>
            <p>Cep Telefonu</p>
            <p>Akıllı Saat</p>
            <p>Akıllı Bileklik</p>
            <p>Leptop</p>
            <p>Oyun & Konsollar</p>
            <p>Elektrikli Bisiklet</p>
            <p>E-pin ve Cüzdan Kodu</p>
        </div>
        <div>
            <h4>Lüks & Desinger</h4>
            <p>Lüks Giyim</p>
            <p>Lüks Ayakkabı</p>
            <p>Lüks Çanta</p>
        </div>
      </div>
      <div className='imgClass'>
        <div className='ww'>
        <div className='imgCss'>
          <img  alt='s' src='https://cdn.dsmcdn.com/mnresize/500/247/ty567/campaign/banners/original/611090/0c84339d0f_0_detay.jpg' className='imgCss2'></img>
        </div>
        <div className='imgCss'>
          <img  alt='s' src='https://cdn.dsmcdn.com/mnresize/500/247/ty567/campaign/banners/original/611090/0c84339d0f_0_detay.jpg' className='imgCss2' ></img>
      </div>
        </div>
      </div>

    </div>
  )
}

export default MenPopup;
